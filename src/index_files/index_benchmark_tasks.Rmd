...
+ **2. Test [scaling and batch detection limits of metrics with simulated batch effects with increased batch strength](metric_scaling_simulations.html).**  
...
+ **4. Test the metrics ability to detect randomness in a dataset with a [moderate](metric_scaling_random_pbmc_roche.html) and a [strong](metric_scaling_random_csf_patient_random.html) batch effect.**  
...
+ **5. Find [which metrics are correlated, which task they correspond in, which not and why](metric_scaling_random_pbmc_roche.html).**  
We wanted to test the metrics scaling with increases randomness regarding the batch effect. This serves as a negative control as metrics should report no batch effect, if batch labels are distributed randomly. To do so we randomly permute an increasing number of batch label from 0% - 100% without touching the underlying data. 
